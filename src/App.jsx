...
const workVideos = Array.from(
  { length: 10 },
  (_, i) => `/work/video-${String(i + 1).padStart(2, "0")}.mp4`
);
...
const problems = [
  {
    ...
    text: "Publicás contenido con valor, pero la gente no llega a verlo porque el inicio no captura atención lo suficientemente rápido.",
  },
  {
    icon: CircleDot,
    title: "Sin sistema claro",
    text: "Cada video se siente como empezar desde cero: ideas sueltas, guiones improvisados y edición sin una estructura repetible.",
  },
  {
    icon: Clock,
    title: "Tiempo perdido",
    text: "Pasás horas pensando ideas, grabando, editando y corrigiendo piezas que después no reflejan el esfuerzo invertido.",
  },
];
...
const solutions = [
  {
    icon: TrendingUp,
    title: "Hooks diseñados para retener",
    text: "Convertimos ideas en aperturas fuertes, simples y directas para que el espectador tenga una razón clara para quedarse.",
  },
  {
    icon: PenLine,
    title: "Guiones con estructura",
    text: "Creamos piezas con ritmo, progresión y payoff. No escribimos frases lindas: construimos videos que avanzan.",
  },
  {
    icon: Film,
    title: "Edición lista para publicar",
    text: "Subtítulos, cortes, motion y criterio visual para que cada pieza se sienta nativa de TikTok, Reels y Shorts.",
  },
];
...
const plans = [
  {
    ...
    tag: "Para empezar",
    amount: "8 videos / mes",
    features: ["Research inicial", "Guiones short-form", "Edición vertical", "1 ronda de ajustes"],
  },
  {
    name: "Growth",
    tag: "Más elegido",
    amount: "16 videos / mes",
    features: ["Estrategia mensual", "Research semanal", "Guiones + edición", "Optimización por métricas"],
    featured: true,
  },
  {
    name: "Scale",
    tag: "Sistema completo",
    amount: "30 videos / mes",
    features: ["Content system completo", "Consultoría", "Revisión de analytics", "Prioridad de entrega"],
  },
];
...
function MiniCard({ src, i }) {
  return (
    <motion.div
      ...
      className="work-card relative h-[220px] min-w-[140px] overflow-hidden rounded-[26px] border border-white/10 bg-gradient-to-br from-[#1a1a1a] via-[#101010] to-[#ff5a00]/20 shadow-[0_0_80px_rgba(255,90,0,0.10)] transition-all duration-500 hover:-translate-y-2 hover:border-white/15 hover:shadow-[0_0_100px_rgba(255,90,0,0.16)] sm:h-[300px] sm:min-w-[180px] md:h-[420px] md:min-w-[240px]"
    >
      <video
        src={src}
        autoPlay
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <motion.p className="text-5xl font-extrabold leading-[0.85] tracking-[-0.09em] text-white drop-shadow-[0_0_22px_rgba(255,255,255,0.12)] md:text-6xl">
          {rounded}
        </motion.p>
...
export default function App() {
  ...
  const submitContactForm = (event) => {
    ...
    const subject = encodeURIComponent(`HOOKD Method inquiry - ${contactForm.name || "New contact"}`);
    const body = encodeURIComponent(`Nombre: ${contactForm.name}\nTipo de cuenta: ${contactForm.accountType}\n\n¿Cómo podemos ayudarte?\n${contactForm.message}`);
    window.location.href = `mailto:hello@hookdmethod.com?cc=axel@hookdmethod.com,alex@hookdmethod.com&subject=${subject}&body=${body}`;
    closeContactModal();
  };
    ...
    <div className="min-h-screen overflow-hidden bg-[#050505] text-white selection:bg-black/10 scroll-smooth">
      ...
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-5 md:px-8">
        <a href="#" className="flex items-center gap-3">
          <div className="relative flex h-9 w-9 items-center justify-center sm:h-11 sm:w-11 rounded-2xl border border-white/10 bg-[#050505] text-white shadow-[0_0_40px_rgba(255,255,255,0.12)]">
      ...
      <motion.main key={language} animate={{ opacity: 1 }} initial={{ opacity: 0.92 }} transition={{ duration: 0.2 }} className="relative z-10">
        <section className="mx-auto max-w-7xl px-4 pb-8 pt-4 text-center sm:px-5 md:px-8 md:pb-10 md:pt-12">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}>
            <div className="mx-auto mb-4 inline-flex max-w-[90vw] items-center gap-2 rounded-full border border-[#ff5a00]/20 bg-[#101010] px-3 py-2 text-xs font-medium text-white/70 backdrop-blur sm:px-4 sm:text-sm">
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#ff5a00]"><BadgeCheck className="h-3 w-3 text-white" /></span>
            ...
            <h1 className="mx-auto max-w-[1180px] text-center text-[44px] font-extrabold leading-[0.9] tracking-[-0.07em] text-white sm:text-[64px] md:text-[82px] lg:text-[96px] xl:text-[108px]">
              ...
              <span className="mt-2 block text-white">{isES ? "Mantené a tu audiencia" : "Keep your audience"} <span className="inline-block italic tracking-[-0.09em] text-[#ff5a00]">HOOKD</span></span>
            </h1>
        ...
        <section id="work" className="relative overflow-hidden py-4 md:py-8">
          <div className="marquee-track flex w-max gap-5 px-5 md:gap-7">
            {[...workVideos, ...workVideos].map((video, i) => (
              <MiniCard key={`${video}-${i}`} src={video} i={i} />
            ))}
          </div>
        </section>
              ...
              <div className="relative flex h-full flex-col overflow-hidden rounded-[24px] border border-white/10 bg-[#0b0b0b] p-5 md:p-6">
                ...
                <div className="relative mb-5">
                  ...
                  <h3 className="mt-3 max-w-3xl text-2xl font-medium leading-[0.95] tracking-[-0.06em] md:text-4xl">
                    {isES ? "Si vas a crear," : "If you're going to create,"}<br />{isES ? "hacelo a la manera " : "do it the "}<span className="italic text-[#ff5a00]">HOOKD</span>{isES ? "." : " way."}
                  </h3>
                ...
                <div className="relative mt-auto overflow-hidden rounded-[22px] border border-white/10 bg-[#080808] px-5 pb-4 pt-11">
                  ...
                  <div className="relative h-[190px]">
                    <div className="absolute bottom-7 left-10 right-3 top-3 grid grid-rows-5">{["100%", "75%", "50%", "25%", "0%"].map((label) => <div key={label} className="relative border-t border-white/10"><span className="absolute -left-10 -top-2 text-[11px] font-bold text-white/60">{label}</span></div>)}</div>
                    <svg viewBox="0 0 700 230" preserveAspectRatio="none" className="absolute bottom-7 left-10 right-3 top-3 h-[calc(100%-40px)] w-[calc(100%-52px)] overflow-visible"><defs><linearGradient id="retentionLine" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor="#ffffff" /><stop offset="45%" stopColor="#d4d4d4" /><stop offset="100%" stopColor="#ffffff" /></linearGradient><linearGradient id="retentionFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#ffffff" stopOpacity="0.30" /><stop offset="100%" stopColor="#ffffff" stopOpacity="0" /></linearGradient></defs><motion.path initial={{ d: "M0 26 C90 78 190 118 320 148 C455 174 570 188 700 202 L700 230 L0 230 Z", opacity: 0.35 }} whileInView={{ d: "M0 8 C80 18 150 36 230 50 C345 64 500 70 700 78 L700 230 L0 230 Z", opacity: 1 }} viewport={{ once: true, amount: 0.65 }} transition={{ duration: 1.7, ease: "easeInOut" }} fill="url(#retentionFill)" /><motion.path initial={{ d: "M0 26 C90 78 190 118 320 148 C455 174 570 188 700 202", opacity: 0.25 }} whileInView={{ d: "M0 8 C80 18 150 36 230 50 C345 64 500 70 700 78", opacity: 1 }} viewport={{ once: true, amount: 0.65 }} transition={{ duration: 1.7, ease: "easeInOut" }} fill="none" stroke="url(#retentionLine)" strokeWidth="8" strokeLinecap="round" /></svg>
        ...
        <section id="problems" className="mx-auto max-w-7xl px-5 py-10 md:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-3">{localizedProblems.map((item) => { const Icon = item.icon; return <motion.div whileHover={{ y: -8 }} key={item.title} className="rounded-[24px] border border-white/10 bg-[#101010] p-5 transition-all duration-500 hover:-translate-y-2 hover:border-white/15 hover:bg-[#151515] hover:shadow-[0_0_80px_rgba(255,90,0,0.10)]"><div className="mb-5 flex h-14 w-14 items-center justify-center rounded-[22px] border border-white/10 bg-[#ff5a00] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]"><Icon className="h-6 w-6 stroke-[1.8]" /></div><h3 className="text-xl font-extrabold tracking-[-0.04em]">{item.title}</h3><p className="mt-4 leading-7 text-white/60">{item.text}</p></motion.div>; })}</div>
        </section>
